...
There are two ways to explore this project. You can run the main application to submit an individual natural-language request, or you can run the 23-case real-browser acceptance campaign to follow the system through discovery, approval, capability reuse, and controlled failure scenarios.

**My recommendation is to read the README first, then run the complete acceptance campaign in sequence.** The cases are arranged to show how the components work together: the system first discovers reusable workflows, then approves and replays them for new inputs, and finally exercises the validation and recovery mechanisms that protect execution when something goes wrong.

### 4.1 Run an Individual Request

After configuring the project environment and starting the required demo application, run the main application entry point:

```bash
python -m app.agent.main
```

This path is useful when you want to explore a particular request and observe how the orchestrator decides between selecting an existing approved capability and starting a new discovery workflow.

### 4.2 Run the Complete 23-Case Acceptance Campaign
...
**For the intended end-to-end demonstration, run TC-001 through TC-023 in sequence rather than starting midway through the campaign.** The cases build a useful understanding of the system in stages:

| Cases | What they demonstrate |
|---|---|
| **TC-001–TC-005** | Discover workflows for savings balance, checking balance, email, phone number, and member status. |
| **TC-006–TC-010** | Approve the discovered capabilities and replay them using different member inputs, without repeating discovery. |
| **TC-011** | Select the appropriate approved capability when the same operation is requested in different words. |
| **TC-012–TC-023** | Exercise business outcomes, missing inputs, policy restrictions, checkpoint verification and recovery, human handoff, ambiguous or missing targets and outputs, wrong-member protection, and validator input serialization. |

Running the complete sequence makes the relationship between discovery, approval, and replay easier to follow. It also shows that a passing test does not always mean the requested browser operation completed: several cases pass precisely because the system rejects an unsafe action, reports a recognized business outcome, or stops when it cannot verify the correct result.

### 4.3 Run a Specific Test Case

If you want to inspect one scenario independently—for example, human-assisted checkpoint recovery—you can run:

```bash
python run_acceptance_campaign_23.py --only TC-016
```

Individual cases are useful for focused inspection, but I recommend the complete sequential run for a first walkthrough. The detailed test-case descriptions and expected behaviors are documented in the [Testing and Verification](#39-testing-and-verification) section.